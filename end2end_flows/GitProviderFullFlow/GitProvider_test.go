...
func (suite *GitProRouterTestSuite) TestClassGetGitProivder() {
	...
	authModeType := []string{"ANONYMOUS", "USERNAME_PASSWORD", "SSH"}

	suite.Run("Checking Full Flow of Git Provider with valid payload", func() {

		for _, authMode := range authModeType {
			...
			saveGitProviderRequestDto := GetGitProviderRequestObjectDto(saveGitHostListResponse.Result[0].Id, authMode)
			...
			suite.checkForCiLogs(strconv.Itoa(workflowResponse.CiPipelines[0].Id), triggerCiPipelineResponse.Result.ApiResponse, 2)
